...
def generate_csp(scripts, images, css_files, fonts, objects=None, frames=None, has_inline_scripts=False):
    ...
    # img-src
    img_domains = set(
        get_domain(i) for i in images if get_domain(i)
    )
    if img_domains:
    ...
    # style-src
    css_domains = set(
        get_domain(c) for c in css_files if get_domain(c)
    )
    if css_domains:
    ...
    # font-src
    font_domains = set(
        get_domain(f) for f in fonts if get_domain(f)
    )
    if font_domains:
    ...
    # object-src
    if objects:
        object_domains = set(get_domain(o) for o in objects if get_domain(o))
        if object_domains:
    ...
    if frames:
        ...
        frame_src = set(get_domain(fr) for fr in frames if get_domain(fr))
        if frame_src:
